...
/*
    static func currentLAN(forType type: IPAddressType) -> IPAddress? {
        func getIFAddresses() -> [String] {
            var addresses = [String]()
            
            // Get list of all interfaces on the local machine:
            var baseAddr: ifaddrs
            var ifaddr : UnsafeMutablePointer<ifaddrs> = &baseAddr
            guard sys_getifaddrs(&ifaddr) == 0 else { return [] }
            guard let firstAddr = ifaddr.pointee else { return [] }
            ...
            let _ = sys_freeifaddrs(ifaddr)
            return addresses
        }
        
        guard let ipAddressString = getIFAddresses().first else { return nil }
        return IPAddress(ipAddress: ipAddressString)
    }
*/
